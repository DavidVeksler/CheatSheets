...
def state_map_svg(states: list[StateRow]) -> str:
    ...
    rows_by_name = {row.name: row for row in states}
    gdp_values = [row.gdp_pc for row in states]
    ...
    for name in sorted(geometry):
        ...
        value = rows_by_name[name].gdp_pc
        percentage = (value - gdp_min) / (gdp_max - gdp_min) * 100
        value_label = f"${value:,.0f} GDP per resident"
        paths.append(
            f'<path class="state-shape metric-value" data-state="{label}" '
            f'role="button" tabindex="0" aria-label="{label}: {value_label}" '
            f'style="fill:color-mix(in srgb,var(--map-low) {100 - percentage:.1f}%,var(--map-high) {percentage:.1f}%)" '
            f'd="{"".join(commands)}"><title>{label}: {value_label}</title></path>'
        )
    return "".join(paths)
...
<p class="section-note">Select a system to inspect its institutions. Horizontal position moves from mostly private toward mostly state-directed capital; vertical position moves from market prices toward administrative price formation. Positions are qualitative ideal types, not measured scores.</p>
...
<section class="matrix-section" aria-labelledby="quick"><div class="stamp">QUICK REFERENCE · 6 SYSTEMS × 12 TESTS</div><h2 id="quick">The master matrix</h2><p class="section-note">Read down a column for a system; read across a row to settle a comparison. Hover a row to hold your place.</p>{master}</section>
...
</ol><p><strong>Method:</strong> systems are ideal types. The classification exercise applies four observable tests: productive ownership, price formation, trade posture, and allocation of credit.</p>'''
    return document(
...
<p class="section-note">Choose two traditions to compare six independent questions. Every position is a named category taken from the master matrix, not a score.</p>
...
</svg><figcaption id="lineage-caption">Arrows show intellectual descent, not agreement; each branch contains rival theories.</figcaption></figure>
<ol class="lineage-list"><li><strong>1689 · Locke:</strong> <cite>Two Treatises</cite>, rights and consent.</li><li><strong>1776 · Smith:</strong> <cite>Wealth of Nations</cite>, commerce and institutions.</li><li><strong>1859 · Mill:</strong> <cite>On Liberty</cite>, the harm principle.</li><li><strong>1881 · T. H. Green:</strong> positive capacity and New Liberalism.</li><li><strong>1933+ · New Deal:</strong> “liberal” becomes the U.S. center-left governing label.</li><li><strong>1938 · Colloque Walter Lippmann:</strong> competing programs to renew liberalism.</li><li><strong>1955 · Dean Russell:</strong> proposes “libertarian” for the displaced classical-liberal label.</li><li><strong>1958 · Isaiah Berlin:</strong> negative and positive liberty lecture.</li><li><strong>1962 · Frank Meyer:</strong> fusionist conservative case in <cite>In Defense of Freedom</cite>.</li><li><strong>1971 · Rawls:</strong> egalitarian liberalism in <cite>A Theory of Justice</cite>.</li><li><strong>1974 · Nozick:</strong> rights-based minimal state in <cite>Anarchy, State, and Utopia</cite>.</li><li><strong>1989 · Williamson:</strong> names the Washington Consensus reform list.</li></ol>'''
    ...
    content = f'''
{fingerprint}
<section aria-labelledby="translate"><div class="plate">PLATE I · TRANSLATE BEFORE YOU ARGUE</div><h2 id="translate">What does “liberal” mean here?</h2><p class="section-note">A label is a local dialect. Each cell states the normal center of gravity, not a membership test.</p>{translation}</section>
...
</ol><p><strong>Scope:</strong> transatlantic and Latin American liberal-family labels. Party examples are orientation points verified as of August 2026, not exact embodiments of the column.</p>'''
    return document(
...
<section class="flip-chart" aria-labelledby="flip-chart-title">
  ...
  <figure>
    ...
    <figcaption><strong>Read across each row only.</strong> Its winner is normalized to 100%, so there is no shared axis between accounting bases.</figcaption>
  </figure>
</section>'''
    verdict_rows = [
...
def build_red_blue(states: list[StateRow]) -> str:
    ...
    state_options = "".join(
        f'<option value="{html.escape(row.name, quote=True)}"'
        f'{" selected" if row.name == "California" else ""}>{html.escape(row.name)}</option>'
    ...
    gdp_ranked = sorted(state_only, key=lambda row: row.gdp_pc, reverse=True)
    gdp_low, gdp_high = gdp_ranked[-1], gdp_ranked[0]
    california = next(row for row in state_only if row.name == "California")
    california_gdp_rank = gdp_ranked.index(california) + 1
    map_script = r'''<script>(()=>{
...
function updateDetail(){
  ...
  if(metric==="side") detailRank.textContent="2024 presidential winner: the grouping key for every pooled result.";
...
function buildLegend(metric){
  ...
  const low=document.createElement("span");low.textContent=`Low · ${metrics[metric].format(ranking.at(-1)[1][metric])}`;
  const gradient=document.createElement("i");gradient.className="map-gradient";gradient.setAttribute("aria-hidden","true");
  const high=document.createElement("span");high.textContent=`High · ${metrics[metric].format(ranking[0][1][metric])}`;
...
function render(){
  ...
  paths.forEach(path=>{
    ...
    path.classList.remove("side-trump","side-harris","metric-value","no-data");path.style.removeProperty("fill");
    if(metric==="side") path.classList.add(row.winner==="Trump"?"side-trump":"side-harris");
      ...
      const minimum=ascending[0][1][metric];const maximum=ranking[0][1][metric];
      const percentage=maximum===minimum?50:(value-minimum)/(maximum-minimum)*100;
      path.classList.add("metric-value");
      path.style.fill=`color-mix(in srgb,var(--map-low) ${(100-percentage).toFixed(1)}%,var(--map-high) ${percentage.toFixed(1)}%)`;
    }
...
document.documentElement.classList.add("js");render();setSelected(selected);
    ...
    content = f'''
<section class="map-stage" aria-labelledby="economy-map-title"><div class="data-years"><strong>Data years:</strong> GDP, RPP, ACS, migration, unemployment 2024 · tax burden 2022 · federal balance FFY 2023 · grouping fixed by 2024 presidential winner until the 2028 election.</div><div class="score-label">INTERACTIVE STATE MAP · 50 STATES</div><h2 id="economy-map-title">See the comparison state by state</h2><p class="section-note">Choose a measure, then hover, focus, or select a state for its exact value. Color runs continuously from the lowest to highest state value.</p><div class="map-toolbar no-print"><label for="economy-metric">Map measure<select id="economy-metric"><option value="gdp_pc" selected>GDP per resident</option><option value="side">2024 presidential winner</option><option value="rpp_income">RPP-adjusted median income</option><option value="growth">Real GDP growth, 2019–24</option><option value="migration">Domestic migration</option><option value="unemployment">Unemployment</option><option value="tax">State-local tax burden</option><option value="poverty">Official poverty rate</option><option value="bop">Federal balance per resident</option></select></label><label for="economy-state">Selected state<select id="economy-state">{state_options}</select></label></div><div class="map-grid"><figure class="map-figure"><svg class="economy-map" viewBox="0 0 975 610" role="img" aria-labelledby="economy-map-svg-title economy-map-svg-desc"><title id="economy-map-svg-title">United States GDP per resident map</title><desc id="economy-map-svg-desc">The 50 states are shaded on a continuous scale from the lowest to highest 2024 GDP per resident. With JavaScript, choose another measure and inspect exact state values.</desc>{map_svg}</svg><div class="map-tooltip" id="map-tooltip" role="tooltip" hidden></div><figcaption id="map-legend" class="map-legend"><span>Low · {money(gdp_low.gdp_pc)}</span><i class="map-gradient" aria-hidden="true"></i><span>High · {money(gdp_high.gdp_pc)}</span></figcaption></figure><aside class="map-detail" aria-live="polite"><span class="map-detail-kicker">Selected state</span><strong id="map-detail-name">California</strong><span id="map-detail-winner">Harris won in 2024</span><span class="map-detail-value" id="map-detail-value">{money(california.gdp_pc)}</span><span id="map-detail-rank">2024 GDP per resident · rank {california_gdp_rank} of 50 by value</span></aside></div><noscript><p class="section-note">The default map shows 2024 GDP per resident. The full data for every measure remains available in the ledger below.</p></noscript></section>
...
<section class="stadium" aria-labelledby="score"><div class="score-label">NATIONAL BOX SCORE · SAME WEIGHT, SAME TYPE</div><h2 id="score">Red vs blue state economies: the scoreboard</h2><p class="section-note">Each row answers a different question; no composite “winner” is calculated.</p>{scoreboard}</section>
<section aria-labelledby="flips"><h2 id="flips">The verdict flips when the measure changes</h2>{flips}</section>
<section aria-labelledby="sensitivity"><h2 id="sensitivity">Does the result depend on six close states?</h2><p class="section-note">Remove the six closest high-impact states from both groups, then recompute.</p>{sensitivity}</section>
...
<section aria-labelledby="migration"><h2 id="migration">Where are domestic movers actually going?</h2><p class="section-note">Census net domestic migration, July 1, 2023 to July 1, 2024. Census counts people; IRS migration data can add adjusted gross income.</p>{migration_table}</section>
